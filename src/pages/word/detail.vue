<template>
  ...
  >
    <view v-if="wordDetail" class="container">
      <!-- 单词头部 -->
...
<script setup lang="ts">
  ...
  const currentWordType = ref<string>('recite')
  ...
  const handleNextWord = async () => {
    try {
      const nextWordId = wordDetail.value?.nextWordId

      // 已背诵、收藏 继续展示当前页面
      if (currentWordType.value === 'recited' || currentWordType.value === 'collected') {
        if (nextWordId) {
          wordId.value = nextWordId
          fetchWordDetail(nextWordId, currentWordType.value)
          return
        }
      }
      ...
      if (nextWordId) {
        uni.reLaunch({
          url: `/pages/learn/question?wordId=${nextWordId}&learnedCount=${learnedCount.value + 1}`
        })
      } else {
        uni.showToast({
          title: '已经是最后一个单词了',
  ...
  const goBack = () => {
    if (currentWordType.value === 'collected') {
      uni.reLaunch({
        url: '/pages/word/learned?tab=1'
      })
      return
    }
    if (currentWordType.value === 'recited') {
      uni.reLaunch({
        url: '/pages/word/learned?tab=0'
      })
      return
    }
    if (currentWordType.value === 'review') {
      uni.reLaunch({
        url: '/pages/word/learned?tab=2'
      })
      return
    }
    uni.reLaunch({
  ...
  onLoad(async (options: any) => {
    if (options.wordId) {
      ...
      currentWordType.value = options.wordType || 'recite'
      await ensureLogin()
      fetchWordDetail(wordId.value, currentWordType.value)
    }
    // 获取胶囊位置信息（单位px）
