...
<template>
  ...
  >
    <view class="trial-card" v-for="(item, index) in list" :key="item.hosStandardId">
      <view class="card-header">
        <view class="title-row">
          <view class="icon-wrap" :class="getRandomColorClass(index)">
            <text>{{ item.hosStandardName ? item.hosStandardName.charAt(0) : '医' }}</text>
          </view>
          <text class="title">{{ item.hosStandardName }}</text>
        </view>
      </view>
